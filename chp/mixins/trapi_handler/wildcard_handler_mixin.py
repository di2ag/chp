...
class WildCardHandlerMixin:
    ...
    def _get_wildcard_type(self, message):
        ...
        if wildcard_type == BiolinkEntity(BIOLINK_DRUG):
            return 'drug'
    ...
    def _extract_chp_query(self, message, message_type):
        ...
        for node_key in query_graph.nodes.keys():
            ...
            if node.categories[0] == BiolinkEntity(BIOLINK_PHENOTYPIC_FEATURE) and node.ids[0] in acceptable_target_curies:
                target_id = node_key
                total_nodes += 1
        if total_nodes == 0:
        ...
        for node_key in query_graph.nodes.keys():
            node = query_graph.nodes[node_key]
            if node.categories[0] == BiolinkEntity(BIOLINK_DISEASE) and node.ids[0] in acceptable_disease_curies:
                disease_id = node_key
    ...
    def _construct_trapi_message(self, chp_query, message, query_type=None):
        ...
        if not self.implicit_survival_node:
            ...
            for qedge_key, qedge in qg.edges.items():
                ...
                if kg['edges'][edge_key]['predicate'] == BiolinkEntity(BIOLINK_GENE)_TO_DISEASE_PREDICATE, is_slot=True) and query['query_graph']['nodes'][subject_node]['category'] == BiolinkEntity(BIOLINK_GENE) and query_type == 'gene':
                    kg['edges'].pop(edge_key)
                elif kg['edges'][edge_key]['predicate'] == BIOLINK_CHEMICAL_TO_DISEASE_OR_PHENOTYPIC_FEATURE_PREDICATE, is_slot=True) and query['query_graph']['nodes'][subject_node]['category'] == BiolinkEntity(BIOLINK_DRUG) and query_type == 'drug':
                    kg['edges'].pop(edge_key)
                else:
                    ...
                    if kg['edges'][kg_id]['predicate'] == BiolinkEntity(BIOLINK_DISEASE)_TO_PHENOTYPIC_FEATURE_PREDICATE, is_slot=True):
        ...
        for contrib, wildcard in sorted_wildcard_contributions[:self.max_results]:
            ...
            for qedge_id, qedge in qg.edges.items():
                ...
                if query_type == 'gene' and qedge.predicates[0] == BiolinkEntity(BIOLINK_GENE_TO_DISEASE_PREDICATE, is_slot=True) and qg.nodes[subject_node].categories[0] == BiolinkEntity(BIOLINK_GENE):
                ...
                elif query_type == 'drug' and qedge.predicates[0] == BiolinkEntity(BIOLINK_CHEMICAL_TO_DISEASE_OR_PHENOTYPIC_FEATURE_PREDICATE, is_slot=True) and qg.nodes[subject_node].categories[0] == BiolinkEntity(BIOLINK_DRUG):
                    kedge_id = kg.add_edge(
                            _node_bindings[qedge.subject][0],
                            _node_bindings[qedge.object][0],
                            predicate=qedge.predicates[0],
